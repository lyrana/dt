/*! \file ParticleMeshBoundaryConditions.h
  ...
  UserParticleBoundaryFunctions class, to give it access to 
...
namespace dnt
{
  ...
  */
  template<Ptype PT>    
    ...
    {
      //      template <Ptype PTX>
      //      using CallbackFunctionPtr =  void (*)(Pstruct<PTX>&, py::str&, const int, const double *dx, const double, py::array_t<double>&);
      template <Ptype PTX>      
    ...
    ParticleMeshBoundaryConditions(std::vector<std::string>& species_names, py::object pmesh_M, UserParticleBoundaryFunctions<PT> userPBFs, bool print_flag = false)
      : user_particle_boundary_functions(userPBFs)
      {
        ...
        */
        ...
        //          for (std::map<std::string, int>::iterator it = pBDict.begin(); it != pBDict.end(); ++it)
        //            pBDictInv[it->second] = it->first;
