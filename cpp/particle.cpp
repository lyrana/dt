...
//    int segmentLength;
  ...
  */
  ...
  /*!          
    ...
    \param psa contains the particle data for one neutral species
  ...
  */
  template <Ptype PT>
  void move_neutral_particles_species(SegmentedArrayPair<PT>& psa)
  {

    // Start a loop over the segments
    py::tuple stup = psa.init_inout_loop(true); // 'true' returns data pointers
    auto npSeg = stup[0].cast<py::ssize_t>();
    // For Numpy array return: psegInArray = stup[1].cast<py::array_t<Pstruct<PT>,0>>();
    auto psegIn = stup[1].cast<Pstruct<PT>*>();
    // For Numpy array return: psegOutArray = stup[2].cast<py::array_t<Pstruct<PT>,0>>();
    auto psegOut = stup[2].cast<Pstruct<PT>*>();
    
    py::ssize_t particleCount = 0;
    
    // ipOut counts particles being written to the current 'out' segment.
    py::ssize_t ipOut = 0;
    bool indexChange = false; // Flag to indicate if particle SA indices have
                              // changed. This affects, e.g., trajectories, which use
                              // SA indices to identify particles chosen for
                              // trajectory plots.
    
    while (psegIn != NULL) // Keep looping until we run out of 'in' segments.
      {
        for (auto ipIn = 0; ipIn < npSeg; ipIn++)
          {
            ...
            psegOut[ipOut] = psegIn[ipIn]; // Copy this particle's data from the
            ...
            */
            ...
            int mLastFacet = pmesh_M.NO_FACET;
  ...
  */
  void move_neutral_particles_species(SegmentedArrayPair<Ptype::cartesian_x>& psa)
  {
