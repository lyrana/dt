...
//  {}
  ...
  template <size_t N_CELL_FACETS>
    void advance_charged_species_in_E_field_cartesian_x(py::object &particle_P, py::str species_name, py::object &ctrl, dolfin::Function *neg_E_field, dolfin::Function *external_E_field, bool accel_only)
  {
    using namespace pybind11::literals;

    //std::cout << "Hello from advance_charged_species_in_E_field_cartesian_x@1" << std::endl;
    ...
    auto pMBCs = particle_P.attr("pmesh_bcs").cast<ParticleMeshBoundaryConditions<Ptype::cartesian_x>>(); // (m)
    ...
    if (applySolvedElectricField.is_none())
    ...
    else
      {
        ...
        //std::cout << "advance_charged_species_in_E_field_cartesian_x: applySolvedElectricFieldMap[species_name] = " << applySolvedElectricFieldMap[species_name] << std::endl;
      }
    ...
    if (applyRandomExternalElectricField.is_none())
    ...
    else
      {
        ...
        //std::cout << "advance_charged_species_in_E_field_cartesian_x: applyRandomExternalElectricFieldMap[species_name] = " << applyRandomExternalElectricFieldMap[species_name] << std::endl;
      }
    ...
    Pstruct<Ptype::cartesian_x>* psegIn = psegInCap; // Declares the type of psegIn
    ...
    Pstruct<Ptype::cartesian_x>* psegOut = psegOutCap; // Declares the type of psegOut
    ...
    */
    ...
    //std::cout << "Hello from advance_charged_species_in_E_field_cartesian_x@2" << std::endl;
    
    py::array_t<double>* EsegPtr = nullptr;
    while (psegIn != nullptr) // Keep looping until we run out of "in" segments.
      {
        //std::cout << "Hello from advance_charged_species_in_E_field_cartesian_x@3" << std::endl;
        ...
        if (neg_E_field != nullptr)
          {
            //std::cout << "Hello from advance_charged_species_in_E_field_cartesian_x@3.1" << " applySolvedElectricFieldFlag = " << applySolvedElectricFieldFlag << std::endl;
            if ((applySolvedElectricFieldFlag == true) || (applySolvedElectricFieldMap[species_name] == true))
              //if (applySolvedElectricFieldFlag == true)
              {
                //std::cout << "Hello from advance_charged_species_in_E_field_cartesian_x@3.2" << " npSeg = " << npSeg << std::endl;
                // # std::cout << "interpolate_field_to_points" << std::endl;
                interpolate_field_to_points<Ptype::cartesian_x>(neg_E_field, psegIn, npSeg, negE); // From dolfin_functions.h
                //std::cout << "Hello from advance_charged_species_in_E_field_cartesian_x@4" << std::endl;
                EsegPtr = &negE;
            ...
            if ((applyRandomExternalElectricFieldFlag == true) || (applyRandomExternalElectricFieldMap[species_name] == true))
              {
                ...
                std::cout << "advance_charged_species_in_E_field_cartesian_x: This function needs to be written" << std::endl;                
                // interpolate_random_field_to_points<Ptype::cartesian_x>(external_E_field, psegIn, npSeg, Eext);
                auto EsegProxy = EsegPtr->mutable_unchecked<2>();
        ...
        for (auto ipIn = 0; ipIn < npSeg; ipIn++)
          {
            ...
            if (ipOut == segmentLength)
              {
                ...
                // Pstruct<Ptype::cartesian_x>* psegOut = psegInCap;
                ...
                auto psegOutNumpyArray = segTuple[0].cast<py::array_t<Pstruct<Ptype::cartesian_x>,0>>();
                const auto psegOutNumpyArray_info = psegOutNumpyArray.request();
                const auto psegOut = static_cast<Pstruct<Ptype::cartesian_x>*>(psegOutNumpyArray_info.ptr);
                */
                ...
                if (cFacet != NO_FACET)
                  {
                    ...
                    if (facValue != 0)
                      {
                        ...
                        if ((psegOut[ipOut].bitflags_ & Pstruct<Ptype::cartesian_x>::TRAJECTORY_FLAG) != 0b0) // If this is a trajectory particle.
                          {
                        ...
                        //std::cout << "Hello from advance_charged_species_in_E_field_cartesian_x@6" << std::endl;
                        // # std::cout << "advance_charged_species_in_E_field_cartesian_x" << " calling a bcFunction" << std::endl;
                        ...
                        // # std::cout << "advance_charged_species_in_E_field_cartesian_x" << " returned from bcFunction" << std::endl;                        
                    ...
                    */
                    if (pCellIndexNew == NO_CELL)
                      {
                        if ((psegOut[ipOut].bitflags_ & Pstruct<Ptype::cartesian_x>::DELETE_FLAG) == 0b1)
                          {
            ...
            if ((psegOut[ipOut].bitflags_ & Pstruct<Ptype::cartesian_x>::DELETE_FLAG) == 0b0) // If this particle has not been deleted...
              {
                /*
            ...
            else // This particle has been deleted
              {
                ...
                if (!traj_T.is_none())
                  {
                    if ((psegIn[ipIn].bitflags_ & Pstruct<Ptype::cartesian_x>::TRAJECTORY_FLAG) != 0b0)
                      {
        ...
        auto psegInNumpyArray = segTuple[1].cast<py::array_t<Pstruct<Ptype::cartesian_x>,0>>();
        const auto psegInNumpyArray_info = psegInNumpyArray.request();
        psegIn = static_cast<Pstruct<Ptype::cartesian_x>*>(psegInNumpyArray_info.ptr);
        */
    ...
    void advance_charged_species_in_E_field_cartesian_xy(py::object &particle_P, py::str species_name, py::object &ctrl, dolfin::Function *neg_E_field, dolfin::Function *external_E_field, bool accel_only)
  {
    ...
    //   auto sap_map = particle_P.attr("sap_dict").cast<std::map<std::string, SegmentedArrayPair<PT> *>>();
    //   auto sap = sap_map[std::string(species_name)];
    //   auto sapMap = particle_P.attr("sap_dict").cast<std::map<std::string, SegmentedArrayPair<Ptype::cartesian_xy> *>>();
    auto qom = particle_P.attr("qom")[species_name].cast<double>();
    // Get call-back functions for boundary-crossing particles
    // The following line was used when we were using the Python callbacks:
    // auto bcFunctionDict = particle_P.attr("pmesh_bcs").attr("bc_function_dict").cast<std::map<int, py::dict>>();
        ...
        if (neg_E_field != nullptr)
          {
            ...
            if ((applySolvedElectricFieldFlag == true) || (applySolvedElectricFieldMap[species_name] == true))
              //if (applySolvedElectricFieldFlag == true)
              {
                ...
                interpolate_field_to_points<Ptype::cartesian_xy>(neg_E_field, psegIn, npSeg, negE); // From dolfin_functions.h
            ...
            if ((applyRandomExternalElectricFieldFlag == true) || (applyRandomExternalElectricFieldMap[species_name] == true))
              {
                ...
                // interpolate_random_field_to_points<Ptype::cartesian_xy>(external_E_field, psegIn, npSeg, Eext);
                auto EsegProxy = EsegPtr->mutable_unchecked<2>();
                auto EextProxy = Eext.mutable_unchecked<2>();
                auto nComps = EextProxy.shape(1);
                // Add the interpolated external field to the interpolated solved electric field.
  ...
  // ENDDEF: void advance_charged_species_in_E_field_cartesian_xy(py::object &particle_P, py::str species_name, py::object &ctrl, dolfin::Function *neg_E_field, dolfin::Function *external_E_field, bool accel_only)
  ...
  */
  template <size_t N_CELL_FACETS>    
    ...
    //std::cout << "Hello from advance_neutral_species_cartesian_xyz@0" << std::endl;
    //std::cout << "This is species " << std::string(species_name) << std::endl;
        ...
        for (auto ipIn = 0; ipIn < npSeg; ipIn++)
          {
            ...
            // call the entities() function to get indices of vertices, facets, etc.
            // dolfin::Cell* pcellPtr = new dolfin::Cell(mesh, pCellIndex);
            pcellPtr = new dolfin::Cell(mesh, pCellIndex); // put placement new here
            auto vertices = pcellPtr->entities(0);
