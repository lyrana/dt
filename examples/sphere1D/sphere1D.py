...
# These are abbreviations for physical constansts.
...
print "There are roughtly %.3g cells per electron Debye length" % (scr.lambda_D/scr.dr_av)
print "There are roughtly %.3g Debye lengths in the computational region" % ((scr.rmax-scr.rmin)/scr.lambda_D)
ctrl = DTcontrol_C()
if emitInput is True:
...
#scr.spacecharge_factor = 1.0

#scr.random_external_electric_field_amplitude = 0.1
...
temperature_eV = 0.0 # About 1000K
...
# Don't make density an attribute of Particle_C (yet)
for s in particle_P.species_names:
...
##### FS.3.1. Dynamic electron permittivity (if implicit)
...
#    if scr.random_external_electric_field_amplitude != 0.0:
...
#        PotentialFieldOutputFile << phi_F.function
    ...
    # !!! VTK cannot write a vector field on a 1D grid. VTK no longer in Dolfin.
    # ElectricFieldOutputFile = df_m.File(ctrl.electric_field_output_file)
    # negElectricField_F.function.rename("E", "E_label")
    # ElectricFieldOutputFile << negElectricField_F.function
