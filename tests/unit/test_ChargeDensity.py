...
#        print numberDensityCalc
        ...
        # Compare results
        function_space = numberDensity_F.function_space
...
#        pmesh2d_UM.compute_cell_vertex_dict()
#        pmesh2d_UM.compute_cell_dict()
        ...
        numberDensityElementDegree = 1
        numberDensityFieldType = 'scalar'
        numberDensity_F = Field_C(meshCI=pmesh2d_UM,
                                 element_type=numberDensityElementType,
                                 element_degree=numberDensityElementDegree,
                                 field_type=numberDensityFieldType)
...
#class TestChargeDensity(unittest.TestCase):
    def test_3_compute_charge_density_on_2Dmesh(self):
        ...
        plotTitle = os.path.basename(__file__) + ": " + sys._getframe().f_code.co_name + ": XY mesh"
        # Make the mesh
        mesh2d_UM = UserMesh_C(mi2d_UMI, compute_dictionaries=True, compute_tree=True, plot_flag=plotFlag, plot_title=plotTitle)

        ########## Kinetic particles ##########

        # Create an instance of the DTparticleInput class
        pinPM = self.pinPM = ParticleInput_C()
        # Set up particle variables
        pinPM.precision = np_M.float64

        pinPM.particle_integration_loop = 'loop-on-particles'
        pinPM.position_coordinates = ['x', 'y', 'z'] # determines the particle-storage dimensions
        pinPM.force_components = ['x', 'y', 'z']
        pinPM.force_precision = np_M.float64
        ...
        dynamics = 'implicit'
        plasmaElectronsPM = ParticleSpecies_C(speciesName, charge, mass, dynamics)

        speciesName = 'H_plus'
        charge = 2.0*MyPlasmaUnits_C.elem_charge
        mass = 1.0*MyPlasmaUnits_C.AMU
        dynamics = 'implicit'
        HplusPM = ParticleSpecies_C(speciesName, charge, mass, dynamics)

        # Add these two species to particle input
        pinPM.particle_species = (plasmaElectronsPM, HplusPM,)

        # Make the particle object from pinPM...
        particles_P = Particle_C(pinPM, print_flag=False)

        # ...and attach the particle mesh
        particles_P.pmeshCI = mesh2d_UM
...
#            print 'species_name, particle_list[i] = ', species_name, particle_list[i]
        ...
        chargeDensity_F = Field_C(meshCI=mesh2d_UM,
                                element_type=numberDensityElementType,
                                element_degree=numberDensityElementDegree,
                                field_type=numberDensityFieldType)
