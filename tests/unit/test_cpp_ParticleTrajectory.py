...
# Could call particle_P.initialize_distributions() instead?
        ...
        print("The charged species are", p_P.traj_T.charged_species)
        expectedList = self.trajin.charged_dict['names']
        for sp in p_P.traj_T.charged_species:
            if len(p_P.traj_T.particle_index_list[sp]) == 0:
                print(" Species", sp, "has no particles marked for a trajectory")
                continue
            print("  Species", sp, "has particles", p_P.traj_T.particle_index_list[sp],"marked")
